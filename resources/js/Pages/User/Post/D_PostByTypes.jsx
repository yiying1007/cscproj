...
function PostByType({postsData = [], hasMoreData = false,fetchUrl,filterTypeComponent=false,filterVisibilityComponent=false}){
    ...
    const filteredPosts = posts.filter((post) => {
        ...
        if(filterTypeComponent===true && filterVisibilityComponent === true){
    ...
    return(
        ...
        <div className='index-container'>
            <div className='index-left'>
                <div className='postView-list'>
                    <h5 className='typesLabel'>Post Categories</h5>
                    ...
                    postTypes.map((type) =>{
                        return(
                            <a key={type.id} onClick={() => setTypeFilter(`${type.type_name}`)} className={typeFilter === `${type.type_name}` ? "active" : ""}>{type.type_name}</a>
                        );
                    })
                    )}
                </div>
                    ...
                    <div className="post-border" key={post.id}>
                        ...
                        <div className="post-header">
                            ...
                            <div className="post-header-center">
                                ...
                                <p style={{fontSize:"15px"}}>{new Date(post.created_at).toLocaleString()}</p>
                                ...
                                <Dropdown.Menu>
                                    ...
                                    <Dropdown.Item 
                                        onClick={() =>
                                          ...
                                          disabled={processing}
                                    ><i class='bx bxs-lock-alt' style={{color:"#543A14"}}> Private</i>
                                    </Dropdown.Item>}
                                    ...
                                    <Dropdown.Item 
                                        onClick={() =>
                                          ...
                                          disabled={processing}
                                    ><i class='bx bxs-lock-open-alt' style={{color:"#543A14"}}> Public</i>
                                    </Dropdown.Item>}
                                </Dropdown.Menu>
                            </Dropdown>}
            ...
            {filteredPosts.length === 0 ? (
                ...
                <div className='eventView-list'>
                    <h5 className='eventLabel'><i className='bx bx-calendar-event'></i> Ongoing Event</h5>
                    {filteredPosts.some(post => post.post_type === "Event" &&
                        ...
                        filteredPosts.map((post) => {
                            if (new Date(post.event_start_time) <= new Date() && new Date(post.event_end_time) > new Date()) {
                                return (
                                    <div key={post.id}>
                                        <div className="">
                                        <span style={{fontWeight:"500"}}>{new Date(post.event_start_time).toLocaleDateString()} until {new Date(post.event_end_time).toLocaleDateString()}</span>
                                            <Link className='event-title' href={route('user.postDetail', post)}>
                                                <p>{post.title.length > 100 
            ...
            {filteredPosts.length === 0 ? (
                ...
                <div className='eventView-list'>                        
                <h5 className='eventLabel'><i class='bx bx-calendar-event'></i> Upcoming Event</h5>
                    {filteredPosts.some(post => post.post_type === "Event" && new Date(post.event_start_time) > new Date()) ? (
                        filteredPosts.map((post) => {
                            if (post.post_type === "Event" && new Date(post.event_start_time) > new Date()) {
                                return (
                                    <div key={post.id}>                                            <div className="">
                                        <span style={{fontWeight:"500"}}>{new Date(post.event_start_time).toLocaleDateString()} until {new Date(post.event_end_time).toLocaleDateString()}</span>
                                            <Link className='event-title' href={route('user.postDetail', post)}>
                                                <p>{post.title.length > 100 
                                                    ? post.title.slice(0, 80) + "..." 
