...
//import DetailPost from './D_PostDetailModal';
//import LikePost from './D_LikePost';
import ExpandableText from '../../Components/ExpandableText';



function TargetProfilePost(){

    const { user,posts: initialPosts, hasMorePosts: initialHasMore,auth,postTypes=[] } = usePage().props;
    ...
    const loadMorePosts = async () => {
        if (!user.id) {
            console.error("user id is undefined, skipping fetch.");
            return;
        }
        try {
            const res = await fetch(`loadTargerUserPosts/${user.id}?page=${page + 1}`,{
...
export default TargetProfilePost; 
